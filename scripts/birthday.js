...
async function initApp() {
    try {
        // Load configuration from remote JSON
        ...
        handleDataError();
        alert('Failed to load remote data. Using local configuration.');
    }
}
    ...
    if (birthdayMusic.paused) {
        birthdayMusic.play()
            .then(() => {
                ...
                document.getElementById('musicControl').title = "Pause music";
            })
            .catch(error => console.error("Playback failed:", error));
    } else {
        birthdayMusic.pause();
        document.getElementById('musicControl').innerHTML = '<i class="fas fa-music"></i>';
        document.getElementById('musicControl').title = "Play music";
    }
}
...
async function saveCardAsImage() {
    ...
    try {
        ...
        savePic.crossOrigin = 'anonymous';
        savePic.src = document.getElementById('birthdayPic').src;

        // Wait for image to load
        await new Promise((resolve) => {
            savePic.onload = resolve;
            savePic.onerror = resolve;
        });
        ...
        document.getElementById('saveContainer').classList.add('hidden-for-save');
